...
function sendJson(res, statusCode, payload, headers = {}) {
  ...
  res.writeHead(statusCode, { 'content-type': JSON_TYPE, 'cache-control': 'no-store', ...headers });
  res.end(JSON.stringify(payload));
}

function sendOpenAiError(res, statusCode, message, code, type = 'invalid_request_error', headers = {}) {
...
function validateChatPayload(payload, key, config) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
    return { error: ['Chat completion body must be a JSON object.', 'invalid_request'] };
  }
  if (payload.model !== config.publicModelAlias) {
    return { error: [`Only the '${config.publicModelAlias}' model alias is available.`, 'model_not_allowed'] };
  }
  if (!Array.isArray(payload.messages) || payload.messages.length === 0 || payload.messages.length > config.maxMessages) {
    return { error: [`messages must contain 1 to ${config.maxMessages} text messages.`, 'invalid_messages'] };
  }
  const unsupported = ['tools', 'functions', 'tool_choice', 'response_format', 'modalities', 'audio', 'attachments'];
  if (unsupported.some((field) => field in payload)) {
    return { error: ['Tools, functions, attachments, and multimodal inputs are not enabled in Phase 1.', 'feature_not_enabled'] };
  }
  let inputCharacters = 0;
  for (const message of payload.messages) {
    if (!message || typeof message !== 'object' || !['system', 'user', 'assistant'].includes(message.role) || typeof message.content !== 'string') {
      return { error: ['Each message must use role system, user, or assistant with string content.', 'invalid_messages'] };
    }
    inputCharacters += message.content.length;
  }
  if (inputCharacters > config.maxInputCharacters) {
    return { error: [`Input exceeds the ${config.maxInputCharacters}-character Stage 1 limit.`, 'input_too_large'] };
  }
  ...
  if (!Number.isSafeInteger(requestedOutput) || requestedOutput < 1) {
    return { error: ['max_tokens must be a positive integer.', 'invalid_max_tokens'] };
  }
  const permittedOutput = Math.min(key.max_output, config.absoluteMaxOutput);
  if (requestedOutput > permittedOutput) {
    return { error: [`Requested output exceeds this key's ${permittedOutput}-token maximum.`, 'output_limit_exceeded'] };
  }
  return {
    value: {
...
function createRateLimiter(state, config) {
  return (key) => {
    ...
    const existing = state.rateBuckets.get(key.id) ?? { tokens: capacity, updatedAt: now };
    const replenished = Math.min(capacity, existing.tokens + ((now - existing.updatedAt) * rpm) / 60000);
    if (replenished < 1) {
...
function extractSseMetadata(chunkText, metadata) {
  for (const rawEvent of chunkText.split('\n\n')) {
    ...
    try {
      ...
      if (metadata.ttftMs === null && (typeof delta.reasoning_content === 'string' && delta.reasoning_content.length > 0 || typeof delta.content === 'string' && delta.content.length > 0)) {
        metadata.ttftMs = Date.now() - metadata.startedAtMs;
      }
      if (choice?.finish_reason) metadata.finishReason = choice.finish_reason;
      if (Number.isInteger(event.usage?.completion_tokens)) metadata.reportedOutput = event.usage.completion_tokens;
    } catch {
...
export function createGateway({ config, store, fetchImpl = fetch, logger = console }) {
  const state = { globalActive: 0, activeByKey: new Map(), rateBuckets: new Map() };
  const consumeRate = createRateLimiter(state, config);
  const admit = createAdmission(state, config);

  async function backendHealthy() {
    try {
      const response = await fetchImpl(`${config.llamaBaseUrl}/models`, { signal: AbortSignal.timeout(1500) });
      return response.ok;
    } catch {
      return false;
    }
  ...
  async function record(storeEvent) {
    try {
      ...
      logger.error(JSON.stringify({ level: 'error', message: 'gateway_event_write_failed', error: error.message }));
    }
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/healthz') {
      ...
      sendJson(res, healthy ? 200 : 503, { status: healthy ? 'ok' : 'degraded', model: config.publicModelAlias, backend: healthy ? 'reachable' : 'unreachable' });
      return;
    }

    if (req.method === 'GET' && url.pathname === '/metrics') {
      if (!secureEqual(req.headers['x-admin-token'], config.adminToken)) {
        sendOpenAiError(res, 401, 'Administrator authentication is required.', 'admin_auth_required', 'authentication_error');
        return;
      }
    ...
    if (!key) {
      sendOpenAiError(res, 401, 'Invalid, expired, or revoked API key.', 'invalid_api_key', 'authentication_error');
      return;
    }

    if (req.method === 'GET' && url.pathname === '/v1/models') {
      sendJson(res, 200, { object: 'list', data: [{ id: config.publicModelAlias, object: 'model', owned_by: 'local-gateway' }] });
      return;
    }
    ...
    const finalize = async () => {
      ...
      logger.info(JSON.stringify({ request_id: requestId, key_prefix: key.prefix, status_code: statusCode, outcome, ttft_ms: ttftMs, elapsed_ms: Date.now() - startedAtMs, finish_reason: finishReason }));
    };
    ...
    try {
      if (!consumeRate(key)) {
        ...
        sendOpenAiError(res, 429, 'Request rate limit exceeded. Retry later.', errorCode, 'rate_limit_error', { 'retry-after': '15' });
        return;
      }
      ...
      if (daily.requests >= key.daily_request_limit) {
        ...
        sendOpenAiError(res, 429, 'Daily request allowance reached.', errorCode, 'rate_limit_error');
        return;
      }
      ...
      if (admitted.error) {
        ...
        const message = admitted.error === 'capacity_busy'
          ? 'Interactive capacity is currently full. Retry later.'
          : 'This API key already has an active generation.';
        sendOpenAiError(res, 429, message, errorCode, 'rate_limit_error', { 'retry-after': '15' });
        return;
      }
      reservation = admitted;

      const controller = new AbortController();
      backendTimeout = setTimeout(() => controller.abort(), config.requestTimeoutMs);
      const upstreamHeaders = { 'content-type': 'application/json' };
      if (config.llamaApiKey) upstreamHeaders.authorization = `Bearer ${config.llamaApiKey}`;
      const upstream = await fetchImpl(`${config.llamaBaseUrl}/chat/completions`, {
        method: 'POST',
      ...
      if (!upstream.ok || !upstream.body) {
        ...
        sendOpenAiError(res, 503, 'Inference backend is unavailable.', errorCode, 'server_error');
        return;
      }
      ...
      if (!validation.value.stream) {
        ...
        try {
          ...
          reportedOutput = Number.isInteger(parsed.usage?.completion_tokens) ? parsed.usage.completion_tokens : null;
        ...
        statusCode = 200;
        outcome = 'completed';
        res.writeHead(200, { 'content-type': upstream.headers.get('content-type') || JSON_TYPE, 'cache-control': 'no-store', 'x-request-id': requestId });
        res.end(payloadText);
        completed = true;
        return;
      }
      ...
      let sseBuffer = '';
      const streamMetadata = { startedAtMs, ttftMs: null, reportedOutput: null, finishReason: null };
      while (true) {
      ...
      if (!res.headersSent) {
        ...
        outcome = error.code === 'request_too_large' ? 'request_rejected' : 'gateway_error';
        errorCode = error.code || 'inference_unavailable';
        sendOpenAiError(res, statusCode, error.statusCode ? error.message : 'Inference backend is unavailable.', errorCode, error.statusCode ? 'invalid_request_error' : 'server_error');
      } else if (!res.writableEnded) {
        res.end();
      }
      logger.error(JSON.stringify({ request_id: requestId, message: 'gateway_request_failed', error: error.message }));
    } finally {
  ...
  return {
    ...
    state,
    start() {
      return new Promise((resolve) => server.listen(config.port, config.bindHost, resolve));
    },
    stop() {
