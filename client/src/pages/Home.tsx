...
const initialConfig: TestConfig = {
  ...
  prompt:
    "Explain, in three practical points, why concurrent LLM requests need bounded queueing.",
  uniqueSuffix: true,
};
...
function readableGatewayError(status: number, rawBody: string) {
  ...
  return {
    code: `http_${status}`,
    message: rawBody.replace(/\s+/g, " ").slice(0, 180) || `HTTP ${status}`,
  };
}
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="grid h-10 w-10 place-items-center rounded-xl border border-orange-300/35 bg-orange-400/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
        <img
          className="h-7 w-7"
          src="/manus-storage/phase0-calibration-mark_addb599f.png"
          alt="Phase 0 calibration mark"
        />
      </div>
      <div className="relative">
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold tracking-[-0.04em] text-stone-100">
            PHASE 0
          </span>
          <span className="h-3.5 w-px bg-orange-400/70" />
          <span className="text-sm font-bold tracking-[-0.04em] text-orange-300">
            LOAD LAB
          </span>
        </div>
        <span className="absolute bottom-[-3px] right-0 h-px w-[61px] bg-orange-400/85" />
        <p className="mono mt-0.5 text-[10px] uppercase tracking-[0.16em] text-stone-500">
          Local LLM capacity observation
        </p>
      </div>
    </div>
  );
...
function MetricReadout({
  ...
  return (
    <div className="min-w-0 border-l border-stone-700/70 pl-4 first:border-l-0 first:pl-0">
      ...
      <p
        className={`mono mt-1 truncate text-lg font-semibold tracking-[-0.04em] ${accent ? "text-orange-300" : "text-stone-100"}`}
      >
        {value}
      </p>
...
function CapacityMeter({
  ...
  const queued = users.filter(
    user => user.status === "queued" || user.status === "waiting"
  ).length;
    ...
    <div className="rounded-2xl border border-stone-700/80 bg-stone-950/55 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mono mt-1 text-sm font-semibold text-stone-100">
            {active} active · {queued} waiting · {complete} complete
          </p>
        </div>
        <span className="mono rounded-md border border-stone-700 bg-stone-900 px-2 py-1 text-[10px] text-stone-400">
          {laneCount} lanes drawn
        </span>
      </div>
      <div className="mt-4 grid grid-cols-4 gap-1.5 sm:grid-cols-6">
        {Array.from({ length: laneCount }, (_, index) => {
          ...
          return (
            <div
              className="rounded-sm border border-stone-700/80 bg-stone-900/70 p-1"
              key={index}
            >
              <div className={`h-6 rounded-[2px] ${color}`} />
              <p className="mono mt-1 text-center text-[8px] text-stone-500">
...
function ChannelCard({
  ...
  const generationRate =
    user.generationTokensPerSecond ??
    (user.completionTokens && user.elapsedMs && user.firstTokenMs
      ? user.completionTokens /
        Math.max((user.elapsedMs - user.firstTokenMs) / 1_000, 0.001)
      : undefined);
  const progress =
    user.status === "completed" || user.status === "protected"
      ? 100
      : user.status === "streaming"
        ? 58
        : user.status === "waiting"
          ? 24
          : user.status === "error" || user.status === "cancelled"
            ? 100
            : 8;
    ...
    <article className="instrument-panel sweep-in relative overflow-hidden rounded-2xl border border-stone-700/70 p-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="mono text-xs font-semibold text-stone-100">
            VU-{String(user.id).padStart(2, "0")}
          </span>
          {user.keyPrefix && (
            <span className="mono text-[9px] text-stone-500">
              {user.keyPrefix}
            </span>
          )}
        </div>
        <span
          className={`mono rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.08em] ${statusStyle[user.status]}`}
        >
          {user.status}
        </span>
      </div>
      ...
      <div className="mt-4 h-1 overflow-hidden rounded-full bg-stone-800">
        <div
          className={`h-full rounded-full transition-all duration-200 ${user.status === "error" ? "bg-red-400" : user.status === "cancelled" ? "bg-stone-500" : user.status === "protected" ? "bg-teal-300" : "bg-orange-400"}`}
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3">
        <div>
          ...
          <p className="mono mt-1 text-xs text-stone-200">
            {formatMs(user.firstTokenMs)}
          </p>
        </div>
        <div>
          <p className="panel-label">Response start</p>
          <p className="mono mt-1 text-xs text-stone-200">
            {formatMs(user.responseStartMs)}
          </p>
        </div>
        <div>
          <p className="panel-label">Elapsed</p>
          <p className="mono mt-1 text-xs text-stone-200">
            {formatMs(user.elapsedMs)}
          </p>
        </div>
        <div>
      ...
      {user.firstTokenMs !== undefined && (
        <div className="mt-4 flex items-center gap-2">
          ...
          <div className="h-1 flex-1 overflow-hidden rounded-full bg-stone-800">
            <div
              className={`h-full rounded-full ${metricTone(user.firstTokenMs, baselineTtft ?? user.firstTokenMs)}`}
              style={{
                width: `${Math.min(100, 20 + (user.firstTokenMs / Math.max(baselineTtft ?? user.firstTokenMs, 1)) * 28)}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="mt-4 space-y-3">
        {user.policyResult && (
          <div className="rounded-lg border border-teal-300/20 bg-teal-300/[0.05] px-3 py-2">
            <p className="mono text-[10px] leading-4 text-teal-100/90">
              Expected gateway protection verified: {user.policyResult}
            </p>
          </div>
        )}
        {user.reasoning && (
          <div>
            <p className="panel-label mb-1.5 text-amber-200/75">
              Reasoning stream
            </p>
        ...
        <div>
          ...
          <div className="mono max-h-80 min-h-20 overflow-y-auto rounded-xl border border-stone-700/60 bg-stone-950/45 p-3 text-[11px] leading-5 text-stone-400">
            {user.error ? (
              <span className="text-red-200">{user.error}</span>
            ) : (
              user.output || (
                <span className="text-stone-600">
                  {user.reasoning
                    ? "No final-answer token arrived before the request ended."
                    : "Awaiting streamed output…"}
                </span>
              )
            )}
          </div>
        ...
        {user.finishReason === "length" && (
          <div className="rounded-lg border border-amber-300/20 bg-amber-300/[0.05] px-3 py-2">
            <p className="mono text-[10px] leading-4 text-amber-100/80">
              Model stopped at the requested token cap. Increase Max output only
              if the remaining context permits it.
            </p>
          </div>
        )}
        {user.finishReason && user.finishReason !== "length" && (
          <p className="mono text-[10px] text-stone-500">
            Server completion reason: {user.finishReason}
          </p>
        )}
      </div>
    </article>
...
export default function Home() {
  ...
  const [events, setEvents] = useState<EventItem[]>([
    {
      ...
      tone: "neutral",
      message:
        "Configure an OpenAI-compatible endpoint, then launch a bounded run.",
    },
  ]);
  ...
  const metrics = useMemo(() => {
    ...
    const protectedCount = users.filter(
      user => user.status === "protected"
    ).length;
    const errorCount = users.filter(user => user.status === "error").length;
    const cancelledCount = users.filter(
    ...
    const ttfts = completed.flatMap(user =>
      user.firstTokenMs === undefined ? [] : [user.firstTokenMs]
    );
    const elapsed = completed.flatMap(user =>
      user.elapsedMs === undefined ? [] : [user.elapsedMs]
    );
    const completionTokens = completed.flatMap(user =>
      user.completionTokens === undefined ? [] : [user.completionTokens]
    );
    ...
    return {
      ...
      totalCompletionTokens: completionTokens.reduce(
        (sum, value) => sum + value,
        0
      ),
  ...
  const applyGatewayTestMode = (gatewayTestMode: GatewayTestMode) => {
    ...
    setConfig(current => {
      if (gatewayTestMode === "raw-capacity")
        return { ...current, ...rawLlamaDefaults, gatewayTestMode };
      if (gatewayTestMode === "gateway-smoke")
        return {
          ...current,
          ...gatewayDefaults,
          gatewayTestMode,
      ...
      if (gatewayTestMode === "same-key-limit")
        return {
          ...current,
          ...gatewayDefaults,
          gatewayTestMode,
  ...
  const runUser = async (id: number, currentRunId: number, apiKey: string) => {
    ...
    updateUser(id, {
      status: "waiting",
      startedAt: start,
      keyPrefix: apiKey ? keyPrefix(apiKey) : undefined,
    });
    addEvent(
      "neutral",
      `VU-${String(id).padStart(2, "0")} dispatched to the model server.`
    );
    ...
    try {
      ...
      if (!response.ok) {
        const detail = readableGatewayError(
          response.status,
          await response.text()
        );
        ...
        if (response.status === 429 && expected === detail.code) {
          ...
          updateUser(id, {
            ...
            policyResult: detail.code,
          });
          addEvent(
            "good",
            `VU-${String(id).padStart(2, "0")} was correctly blocked by the gateway: ${detail.code}.`
          );
          return;
        }
        throw new Error(
      ...
      const consumeLine = (line: string) => {
        ...
        try {
          ...
          const streamToken =
            typeof reasoningToken === "string" && reasoningToken.length > 0
              ? reasoningToken
          ...
          if (streamToken) {
            if (firstTokenMs === undefined) {
              ...
              addEvent(
                "good",
                `VU-${String(id).padStart(2, "0")} received its first ${reasoningToken ? "reasoning" : "answer"} token in ${formatMs(firstTokenMs)}.`
              );
            }
            if (
              typeof reasoningToken === "string" &&
              reasoningToken.length > 0
            ) {
      ...
      if (currentRunId === runId.current) {
        ...
        addEvent(
          "good",
          `VU-${String(id).padStart(2, "0")} completed in ${formatMs(elapsedMs)}.`
        );
      }
    } catch (reason) {
      const elapsedMs = performance.now() - start;
      ...
      addEvent(
        cancelled ? "warn" : "bad",
        `VU-${String(id).padStart(2, "0")} ${cancelled ? "stopped" : "failed"}: ${message}`
      );
    } finally {
  ...
  const startRun = async () => {
    ...
    if (
      ...
      addEvent(
        "bad",
        "Endpoint, model, and prompt are required before a test can start."
      );
      return;
    }
    const pooledKeys = keyPool(config.apiKeyPool);
    if (
      (config.gatewayTestMode === "gateway-smoke" ||
        config.gatewayTestMode === "same-key-limit") &&
      !config.apiKey.trim()
    ) {
      addEvent(
        "bad",
        "Paste one invited-user key before running this gateway test."
      );
      return;
    }
    if (
      config.gatewayTestMode === "global-capacity-limit" &&
      pooledKeys.length < config.virtualUsers
    ) {
      addEvent(
        "bad",
        `Paste ${config.virtualUsers} separate invited-user keys, one per line, before this capacity test.`
      );
      return;
    }
    ...
    const scenarios = Array.from(
      ...
      (_, index) => ({
        id: index + 1,
        status: "queued" as UserStatus,
        output: "",
        reasoning: "",
        keyPrefix:
          config.gatewayTestMode === "global-capacity-limit"
            ? keyPrefix(pooledKeys[index])
            : config.apiKey.trim()
              ? keyPrefix(config.apiKey.trim())
              : undefined,
      })
    );
    setUsers(scenarios);
    ...
    addEvent(
      "neutral",
      `Starting ${config.virtualUsers} virtual users with a ${config.rampMs} ms launch ramp.`
    );

    await Promise.allSettled(
      scenarios.map(async (user, index) => {
        if (index > 0) await wait(index * config.rampMs);
        if (nextRunId !== runId.current) return;
        const apiKey =
          config.gatewayTestMode === "global-capacity-limit"
            ? pooledKeys[user.id - 1]
            : config.apiKey.trim();
    ...
    if (nextRunId === runId.current) {
      ...
      addEvent(
        "neutral",
        "Run settled. Review p50/p95 metrics before changing the next test variable."
      );
    }
  };
  ...
  const resetRun = () => {
    ...
    addEvent(
      "neutral",
      "Run evidence cleared. Configuration remains unchanged."
    );
  };
  ...
  return (
    <main
      ...
      style={{
        backgroundImage:
          "linear-gradient(rgba(23,23,23,0.88), rgba(23,23,23,0.97)), url('/manus-storage/phase0-instrument-grain_c5a8e851.png')",
        ...
        <div className="mx-auto flex max-w-[1720px] items-center justify-between gap-5 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-7 lg:flex">
            <div className="text-right">
              ...
              <p className="mono mt-1 text-xs text-stone-300">
            ...
            <div className="text-right">
              ...
              <p className="mono mt-1 flex items-center justify-end gap-1.5 text-xs text-teal-200">
                <Check className="h-3.5 w-3.5" /> bounded browser run
              </p>
            </div>
            <Link
              href="/admin"
      ...
      <div className="mx-auto grid max-w-[1720px] gap-4 px-4 py-4 sm:px-6 lg:grid-cols-[320px_minmax(0,1fr)_320px] lg:gap-5 lg:px-8 lg:py-6">
        <aside className="faceplate h-fit rounded-2xl border border-orange-200/10 lg:sticky lg:top-5">
          ...
          <div className="space-y-5 p-5">
            <label className="block">
              ...
              <select
                className="mono mt-2 h-10 w-full rounded-md border border-stone-700 bg-stone-950/60 px-3 text-xs text-stone-100 outline-none transition-colors focus:border-orange-300"
                value={config.gatewayTestMode}
                onChange={event =>
                  applyGatewayTestMode(event.target.value as GatewayTestMode)
                }
                disabled={isRunning}
              >
                <option value="raw-capacity">
                  Raw llama.cpp capacity test
                </option>
                <option value="gateway-smoke">
                  Gateway smoke check — 1 key, 1 user
                </option>
                <option value="same-key-limit">
                  Gateway fairness check — 1 key, 2 users
                </option>
                <option value="global-capacity-limit">
                  Gateway capacity check — 5 keys, 5 users
                </option>
              </select>
              <span className="mono mt-1.5 block text-[10px] leading-4 text-stone-500">
                Choose a recipe and the dashboard fills the safe endpoint, model
                name, users, ramp, and output cap.
              </span>
            </label>
            ...
            <label className="block">
              ...
              <div className="mt-2 flex gap-2">
                <Input
                  className="mono h-10 border-stone-700 bg-stone-950/60 text-xs text-stone-100 placeholder:text-stone-600"
                ...
                <Button
                  variant="outline"
                  size="icon"
                  className="h-10 shrink-0 border-stone-700 bg-stone-900/60 text-stone-300 hover:bg-stone-800 hover:text-stone-100"
                  onClick={copyEndpoint}
                  disabled={isRunning}
                  aria-label="Copy completion URL"
                >
                  <Copy className="h-4 w-4" />
                </Button>
              </div>
              <span className="mono mt-1.5 block text-[10px] leading-4 text-stone-500">
                The dashboard adds `/chat/completions` automatically.
              </span>
            </label>
            ...
            <label className="block">
              <span className="panel-label">
                ...
                <span className="normal-case tracking-normal text-stone-600">
                  {config.gatewayTestMode === "raw-capacity"
                    ? "optional for local test"
                    : "one invited-user key"}
                </span>
              </span>
              <div className="relative mt-2">
                <KeyRound className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-stone-500" />
                <Input
                  className="mono h-10 border-stone-700 bg-stone-950/60 pl-9 pr-10 text-xs text-stone-100 placeholder:text-stone-600"
                  value={config.apiKey}
                  onChange={event =>
                    setConfig(current => ({
                      ...current,
                ...
                <button
                  type="button"
                  className="absolute right-3 top-2.5 text-[10px] font-semibold uppercase tracking-[0.1em] text-stone-500 transition-colors hover:text-stone-200"
              ...
              <span className="mono mt-1.5 block text-[10px] leading-4 text-stone-500">
                Held only in this browser session. Paste a customer key, never
                an admin token or master secret.
              </span>
            </label>
            ...
            {config.gatewayTestMode === "global-capacity-limit" && (
              <label className="block">
                <span className="panel-label">
                  Five separate invited-user keys
                </span>
                <Textarea
                  className="mono mt-2 min-h-28 resize-y border-stone-700 bg-stone-950/60 text-xs leading-5 text-stone-100 placeholder:text-stone-600"
                  value={config.apiKeyPool}
                  onChange={event =>
                    setConfig(current => ({
                      ...current,
                      apiKeyPool: event.target.value,
                    }))
                  }
                  placeholder={
                    "gma_live_first...\ngma_live_second...\ngma_live_third...\ngma_live_fourth...\ngma_live_fifth..."
                ...
                />
                <span className="mono mt-1.5 block text-[10px] leading-4 text-stone-500">
                  Paste one customer key per line. The first four should run;
                  the fifth should show a successful gateway block.
                </span>
              </label>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block col-span-2">
                ...
                <Input
                  className="mono mt-2 h-10 border-stone-700 bg-stone-950/60 text-xs text-stone-100 placeholder:text-stone-600"
                  value={config.model}
                  onChange={event =>
                    setConfig(current => ({
                      ...current,
              ...
              <label className="block">
                ...
                <Input
                  className="mono mt-2 h-10 border-stone-700 bg-stone-950/60 text-xs text-stone-100"
                  type="number"
                  min={1}
                  max={12}
              ...
              <label className="block">
                ...
                <div className="relative mt-2">
                  <Input
                    className="mono h-10 border-stone-700 bg-stone-950/60 pr-8 text-xs text-stone-100"
                    type="number"
                    min={0}
                    max={10_000}
                    step={100}
                    ...
                    disabled={isRunning}
                  />
                  <span className="mono pointer-events-none absolute right-2.5 top-3 text-[10px] text-stone-500">
                    ms
                  </span>
                </div>
              </label>
              <label className="block">
                ...
                <div className="relative mt-2">
                  <Input
                    className="mono h-10 border-stone-700 bg-stone-950/60 pr-9 text-xs text-stone-100"
                    type="number"
                    min={8}
                ...
                <p className="mono mt-1.5 text-[10px] leading-4 text-stone-500">
                  Up to 8K requested. Actual generation is limited by remaining
                  model context after prompt tokens.
                </p>
              </label>
              <label className="block">
                  ...
                  <Input
                    ...
                    onChange={event =>
                      setConfig(current => ({
                        ...current,
                        timeoutMs: clamp(
                          Number(event.target.value) || 5_000,
                          5_000,
                          600_000
                        ),
                      }))
            ...
            <label className="block">
              ...
              <Textarea
                className="mono mt-2 min-h-20 resize-y border-stone-700 bg-stone-950/60 text-xs leading-5 text-stone-100 placeholder:text-stone-600"
            ...
            <label className="block">
              ...
              <Textarea
                className="mono mt-2 min-h-32 resize-y border-stone-700 bg-stone-950/60 text-xs leading-5 text-stone-100 placeholder:text-stone-600"
                value={config.prompt}
                onChange={event =>
                  setConfig(current => ({
                    ...current,
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-stone-700/70 bg-stone-950/40 p-3 transition-colors hover:border-stone-600">
              <input
                ...
                type="checkbox"
                checked={config.uniqueSuffix}
                onChange={event =>
                  setConfig(current => ({
                    ...current,
                    uniqueSuffix: event.target.checked,
                  }))
                }
                disabled={isRunning}
              />
              <span>
                <span className="mono block text-[11px] font-medium text-stone-200">
                  Make users independent
                </span>
                <span className="mono mt-1 block text-[10px] leading-4 text-stone-500">
                  Adds a user-specific suffix. Turn off only for shared-prefix
                  cache tests.
                </span>
              </span>
            </label>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              {isRunning ? (
                <Button
                  className="col-span-2 h-11 bg-red-500 text-white shadow-[0_10px_24px_rgba(239,68,68,0.18)] hover:bg-red-400"
                  onClick={stopRun}
                >
                  <CircleStop className="mr-2 h-4 w-4" /> Stop active run
                </Button>
              ) : (
                <Button
                  className="col-span-2 h-11 bg-orange-400 text-stone-950 shadow-[0_10px_24px_rgba(240,93,35,0.22)] hover:bg-orange-300"
                  onClick={startRun}
                >
                  <Play className="mr-2 h-4 w-4 fill-current" /> Launch{" "}
                  {config.virtualUsers} users
                </Button>
              )}
              <Button
                variant="outline"
                className="h-10 border-stone-700 bg-stone-900/40 text-stone-300 hover:bg-stone-800 hover:text-stone-50"
                onClick={resetRun}
                disabled={isRunning || users.length === 0}
              >
                <RotateCcw className="mr-2 h-4 w-4" /> Clear evidence
              </Button>
              <Button
                variant="outline"
                className="h-10 border-stone-700 bg-stone-900/40 text-stone-300 hover:bg-stone-800 hover:text-stone-50"
                ...
                disabled={isRunning}
              >
                <TimerReset className="mr-2 h-4 w-4" /> Reset plan
              </Button>
            </div>
          </div>
        </aside>
        ...
        <section className="min-w-0 space-y-4 lg:space-y-5">
          <div className="instrument-panel relative overflow-hidden rounded-2xl border border-stone-700/70">
            <div
              ...
              style={{
                backgroundImage:
                  "url('/manus-storage/phase0-capacity-lines_28efcafd.png')",
                backgroundSize: "cover",
            ...
            <div className="relative p-5 sm:p-6">
              <div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-start">
                <div className="max-w-md">
                  <p className="panel-label text-orange-300/80">
                    Live observation field
                  </p>
                  <h2 className="mt-1.5 text-xl font-semibold tracking-[-0.05em] text-stone-50 sm:text-2xl">
                    Live capacity profile
                  </h2>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-400">
                    Read the lane bus first. The summary below is evidence from
                    independent streaming requests, not a theoretical speed
                    claim.
                  </p>
                </div>
                <div className="flex items-center gap-3 self-start rounded-xl border border-stone-700/80 bg-stone-950/55 px-3 py-2 xl:self-auto">
                  <div
                    className={`h-2.5 w-2.5 rounded-full ${isRunning ? "bg-orange-400 animate-pulse" : "bg-stone-600"}`}
                  />
                  <div>
                    <p className="panel-label">Run state</p>
                    <p className="mono mt-0.5 text-xs font-medium text-stone-200">
                      {isRunning
                        ? "IN PROGRESS"
                        : users.length
                          ? "SETTLED"
              ...
              <div className="mt-6 grid gap-4 border-t border-stone-700/70 pt-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(300px,0.8fr)]">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-5 rounded-2xl border border-stone-700/80 bg-stone-950/40 p-4">
                  <MetricReadout
                    label="Active demand"
                    value={`${metrics.active} / ${config.virtualUsers}`}
                    hint="queued + waiting + streaming"
                    accent={metrics.active > 0}
                  />
                  <MetricReadout
                    label="p50 TTFT"
                    value={formatMs(metrics.p50Ttft)}
                    hint="first streamed token"
                  />
                  <MetricReadout
                    label="p95 TTFT"
                    value={formatMs(metrics.p95Ttft)}
                    hint="tail interactive latency"
                  />
                  <MetricReadout
                    label="p95 elapsed"
                    value={formatMs(metrics.p95Elapsed)}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-1 text-base font-semibold tracking-[-0.04em]">
                    Each card is one independent streaming request
                  </h2>
                </div>
              </div>
            ...
            <div className="min-h-[380px] p-4 sm:p-5">
              {users.length === 0 ? (
                <div className="grid min-h-[340px] place-items-center rounded-2xl border border-dashed border-stone-700 bg-stone-950/30 p-8 text-center">
                  <div className="max-w-md">
                    <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl border border-orange-300/20 bg-orange-400/10">
                      <UsersRound className="h-5 w-5 text-orange-300" />
                    </div>
                    <h3 className="mt-4 text-lg font-semibold tracking-[-0.04em]">
                      No request channels yet
                    </h3>
                    <p className="mt-2 text-sm leading-6 text-stone-500">
                      Start with one virtual user to capture a baseline. Then
                      run the exact same test at two users before you change the
                      prompt, context, or output cap.
                    </p>
                  </div>
                </div>
              ) : (
            ...
            <div className="instrument-panel rounded-2xl border border-stone-700/70 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-1 text-base font-semibold tracking-[-0.04em]">
                    Do not treat one number as capacity
                  </h2>
                </div>
              </div>
              <dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-4">
                <div>
                  <dt className="panel-label">Completed</dt>
                  <dd className="mono mt-1 text-xl font-semibold text-teal-200">
                    {metrics.completed}
                  </dd>
                </div>
                <div>
                  <dt className="panel-label">Failed / stopped</dt>
                  <dd
                    className={`mono mt-1 text-xl font-semibold ${metrics.failed ? "text-red-200" : "text-stone-200"}`}
                  >
                    {metrics.failed}
                  </dd>
                </div>
                <div>
                  <dt className="panel-label">Gateway blocks</dt>
                  <dd className="mono mt-1 text-xl font-semibold text-orange-200">
                    {metrics.protectedCount}
                  </dd>
                </div>
                <div>
                  <dt className="panel-label">p50 elapsed</dt>
                  <dd className="mono mt-1 text-sm text-stone-200">
                    {formatMs(metrics.p50Elapsed)}
                  </dd>
                </div>
                <div>
                  <dt className="panel-label">Server-reported output</dt>
                  <dd className="mono mt-1 text-sm text-stone-200">
                    {metrics.reportingUsers
                      ? `${metrics.totalCompletionTokens} tok`
                      : "not reported"}
                  </dd>
                </div>
              </dl>
            ...
            <div className="instrument-panel rounded-2xl border border-stone-700/70 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-1 text-base font-semibold tracking-[-0.04em]">
                    One changed variable per run
                  </h2>
                </div>
              </div>
              <p className="mt-4 text-sm leading-6 text-stone-400">
                Record the baseline first. Then change only one of users, ramp,
                server slots, context size, or maximum output. This is how you
                find a reliable operating limit instead of guessing.
              </p>
              <a
                className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-orange-300 transition-colors hover:text-orange-200"
                href="https://github.com/ggml-org/llama.cpp/blob/master/tools/server/README.md"
                target="_blank"
                rel="noreferrer"
              >
                Review server slot controls <ArrowUpRight className="h-4 w-4" />
              </a>
            </div>
          </div>
        </section>
        ...
        <aside className="instrument-panel relative min-h-[300px] overflow-hidden rounded-2xl border border-stone-700/70 lg:sticky lg:top-5 lg:max-h-[calc(100vh-40px)]">
          <div
            ...
            style={{
              backgroundImage:
                "url('/manus-storage/phase0-run-tape-pattern_56869dd9.png')",
              backgroundSize: "cover",
              backgroundPosition: "center",
            }}
          />
          <div className="relative flex h-full flex-col">
                ...
                return (
                  ...
                  >
                    {index !== events.length - 1 && (
                      <span className="absolute left-[3px] top-3 h-[calc(100%-6px)] w-px bg-stone-700/80" />
                    )}
                    <span
                      className={`relative z-10 mt-1.5 h-2 w-2 rounded-full ${tone}`}
                    />
                    <div>
                      <p className="mono text-[10px] text-stone-500">
                        {event.timestamp}
              ...
              {events.length < 4 && (
                <div className="mt-1 border-t border-dashed border-stone-700/70 pt-4">
                  ...
                  {[
                    ...
                    <div
                      className="mono mt-3 flex items-center gap-2 text-[10px] text-stone-600"
                      key={label}
                    >
            ...
            <div className="border-t border-stone-700/70 p-4">
              <div className="mb-3 grid grid-cols-3 gap-2">
                <div className="rounded-lg border border-stone-700/70 bg-stone-950/45 p-2">
                  <p className="panel-label">Active</p>
                  <p className="mono mt-1 text-sm text-orange-300">
                    {metrics.active}
                  </p>
                </div>
                <div className="rounded-lg border border-stone-700/70 bg-stone-950/45 p-2">
                  <p className="panel-label">Done</p>
                  <p className="mono mt-1 text-sm text-teal-200">
                    {metrics.completed}
                  </p>
                </div>
                <div className="rounded-lg border border-stone-700/70 bg-stone-950/45 p-2">
                  <p className="panel-label">Faults</p>
                  <p className="mono mt-1 text-sm text-red-200">
                    {metrics.failed}
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2 rounded-xl border border-stone-700/70 bg-stone-950/45 p-3">
                ...
                <p className="mono text-[10px] leading-4 text-stone-500">
                  This tool is a browser-side Phase 0 instrument. Keep the key
                  ephemeral, test synthetic prompts, and do not call a public
                  production endpoint until a protected gateway exists.
                </p>
              </div>
            </div>
          </div>
