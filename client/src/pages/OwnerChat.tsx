...
export default function OwnerChat() {
  ...
  return (
    ...
    {projectInfoOpen && <div className="fixed inset-0 z-50 flex items-end bg-[#20232d]/30 p-0 backdrop-blur-[2px] sm:items-center sm:justify-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="mattr-chat-info-title">
      ...
      <section className="relative max-h-[calc(100dvh-1rem)] w-full overflow-y-auto rounded-t-[24px] border border-white/80 bg-white px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-5 shadow-[0_22px_60px_rgba(30,35,52,0.22)] sm:max-w-[560px] sm:rounded-[22px] sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><BrandMark /><span className="text-[12px] font-semibold text-[#4e5670]">Mattr Chat</span></div><h2 id="mattr-chat-info-title" className="mt-3 text-[22px] font-semibold tracking-[-0.035em] text-[#252832]">Built from a local model.</h2></div><button onClick={() => setProjectInfoOpen(false)} className="grid h-10 w-10 shrink-0 place-items-center rounded-[10px] text-[#747985] transition-colors hover:bg-[#f1f2f5]" aria-label="Close"><X className="h-4 w-4" /></button></div>
        <p className="mt-3 text-[13px] leading-6 text-[#676d7b]">Mattr Chat began with an AI model running on my own local machine. This project is about turning it into a small internet-accessible service and learning every layer around it.</p>
        <div className="mt-5 grid gap-2.5 sm:grid-cols-2"><div className="rounded-[14px] border border-[#e4e7ef] bg-[#f8f9fc] p-3"><p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#70799e]">Run locally</p><p className="mt-1.5 text-[12px] leading-5 text-[#555b69]">The model runs on my own hardware, not a hosted AI platform.</p></div><div className="rounded-[14px] border border-[#e4e7ef] bg-[#f8f9fc] p-3"><p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#70799e]">Serve real users</p><p className="mt-1.5 text-[12px] leading-5 text-[#555b69]">I built the layer connecting the chat and protected API to the model.</p></div><div className="rounded-[14px] border border-[#e4e7ef] bg-[#f8f9fc] p-3"><p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#70799e]">Context &amp; requests</p><p className="mt-1.5 text-[12px] leading-5 text-[#555b69]">I am learning fair request sharing, conversation context, and KV-cache trade-offs.</p></div><div className="rounded-[14px] border border-[#e4e7ef] bg-[#f8f9fc] p-3"><p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#70799e]">Secure the service</p><p className="mt-1.5 text-[12px] leading-5 text-[#555b69]">I am learning API protection, rate limits, access control, and safe request handling.</p></div></div>
        <div className="mt-3 rounded-[14px] border border-[#ece6d7] bg-[#fffdf7] p-3"><p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#8b774b]">Honest limits</p><p className="mt-1.5 text-[12px] leading-5 text-[#5f5a4d]">This is an independent learning project. Capacity is limited, speed and availability can vary, and AI answers can be wrong. Check important answers and do not share sensitive information.</p></div>
        <button onClick={() => setProjectInfoOpen(false)} className="mt-4 flex h-11 w-full items-center justify-center rounded-[11px] bg-[#292d37] text-[13px] font-semibold text-white transition-colors hover:bg-[#373b46]">Got it</button>
      </section>
    </div>}
