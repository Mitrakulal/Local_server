...
The implementation was opened against a temporary router with validation-only placeholder secrets. The revised **Luminous Utility** entry screen rendered a calm light owner-access gate, local-model availability state, focused token entry, and session-only privacy note. A successful local token entry rendered the desktop three-zone conversation workspace: navigation column, central conversation field, and compact model/privacy margin. The session note and Clear/New conversation actions visibly state that conversation text is browser-session scoped. The private load lab and Owner Console retain their separate operational visual systems.
