...
The implementation was opened against a temporary router with validation-only placeholder secrets. The owner-token screen rendered the `Nocturne Ledger` entry experience, and a successful local token entry rendered the desktop three-zone conversation workspace: current-folio rail, central streamed-message folio, and live model/capacity margin. The session note and Clear/New conversation actions visibly state that conversation text is browser-session scoped.
